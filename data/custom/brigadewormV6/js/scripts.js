...
window.onload = function () {
  ...
  var max1 = ['.aware', '.target', '.difficulty', '.scenario', '.alignment1', '.alignment2', '.incarnation', '.age', '.sex', '.gender', '.appearance', '.identity', '.job', '.education', '.homelife', '.shard', '.shardranking', '.oops', '.settinglocation', '.location', '.locationnonearth', '.basesize', '.baseconcealment']
  ...
  for (var i = 0; i < choices.length; i++) {
    ...
    if (choice.classList.contains('tier3') == false && choice.classList.contains('tier2') == false && choice.classList.contains('tier1') == false && choice.classList.contains('tier1st') == false && choice.classList.contains('tier2st') == false && choice.classList.contains('changerendbringerpower') == false && choice.classList.contains('masterendbringerpower') == false) {
      ...
      for (var x = 0; x < counterids.length; ++x) {
        if (selections[i].substr(0, 1) == alphabet[x] && selections[i].substr(1, 3).match(/^[0-9]+$/) != null){
          if (x == 0) {
            ...
            if (prev == 0) {
              prev = 1;
              if (document.getElementById('euib').checked) {
                document.getElementById('check2').innerHTML = document.getElementById('check2').innerHTML + "+" + countervars[x] + " CP -" + countervars[x]*1 + " SP, "
                document.getElementById('check2').value = document.getElementById('check2').value + "+" + countervars[x] + " CP -" + countervars[x]*1 + " SP, "
              } else {
...
document.getElementById('import').onclick = function () {
  ...
  for (var i = 0; i < selections.length; ++i) {
    if (document.getElementById(selections[i]) != null) {
      if (document.getElementById(selections[i]).checked == false)
      document.getElementById(selections[i]).click()
    }
  }
}
...
document.getElementById('b8h1').oninput = function () {
  ...
  if (this.checked == true) {
    if (document.getElementById('1b1s').checked == true || document.getElementById('etl7').checked == true) {
      ...
      if (document.getElementById('wo5e').checked == true) {
        document.getElementById('wo5e').click()
      }
...
document.getElementById('s7sb').oninput = function () {
  multiplerequired(-9, 0, this, ['nx9r'], ['nhgo'])
}
document.getElementById('euib').oninput = function () {
  incompatible(0, 50, this, ['wwb1', 'w0ll', 'aapm', '48m6', '2b7c', 'pr2q', 'whrm', '6s5p', 'l8bx', 'aabi', 'aabc', 'aaaa', 'aagn', 'aabx', 'pjir'])
  if (this.checked == true) {
    document.getElementById('T3convert').classList.add('disabled')
    loop = CPcounter
    for (var i = 0; i < loop; ++i) {
      document.getElementById('removeCP').click()
    }
    convert = document.getElementById('CPconvert').parentElement
    convert.classList.add('visible')
    convert.parentElement.appendChild(convert)
    document.getElementsByName('CPconvert')[0].parentElement.classList.remove('visible')
    for (var i = 0; i < loop; ++i) {
      document.getElementById('addCP').click()
    }
  } else {
...
document.getElementById('whrm').oninput = function () {
  ...
  if (this.checked == true) {
    if (document.getElementById('kht1').checked !=  true && document.getElementById('qpwm').checked !=  true) {
      document.getElementById('appearanceid').disabled = false
      document.getElementsByName('appearanceid')[0].classList.remove('disabled')
      document.getElementsByClassName('appearancesection')[0].classList.remove('visible')
    }
  } else {
    if (document.getElementById('kht1').checked !=  true && document.getElementById('qpwm').checked !=  true) {
      document.getElementById('appearanceid').disabled = true
      document.getElementsByName('appearanceid')[0].classList.add('disabled')
      document.getElementsByClassName('appearancesection')[0].classList.add('visible')
    }
  }
}
document.getElementById('kht1').oninput = function () {
  visible(0, -5, this, ['agesection', 'sexsection', 'civilianidentitysection'], ['ageid', 'sexid', 'civilianidentityid'], ['age', 'sex', 'job'])
...
document.getElementById('icne').oninput = function () {
  incompatible(5, 5, this, ['6zrd', 'vrqv', '87du', 'aaae'])
  if (this.checked) {
    if (document.getElementById('aapq').checked) document.getElementById('aapq').click();
    document.getElementById('aapq').disabled = true
    document.getElementsByName('aapq')[0].classList.add('disabled')
